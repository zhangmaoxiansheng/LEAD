...
class Iterative_Propagate(nn.Module):
    def __init__(self,crop_h,crop_w,mode='c',dropout=False):
        ...
        self.crop_w = crop_w
        self.in_channels = 17 
        self.model_ref0 = nn.Sequential(ConvBlock(self.in_channels,32),
                            ConvBlock(32,32),
                            ConvBlock(32,64),
                            ConvBlock(64,32),
                            ConvBlock(32,16),
        ...
        self.model_ref1 = nn.Sequential(ConvBlock(self.in_channels,32),
                            ConvBlock(32,32),
                            ConvBlock(32,64),
                            ConvBlock(64,32),
                            ConvBlock(32,16),
    ...
    def stage_block(self,features,rgbd,dep_last,stage,outputs):
        ...
        return outputs
    def scale_adjust(self,dep_last,dep):
    ...
    def stage_forward(self,features,rgbd,dep_last,stage):
        ...
        dep = self.scale_adjust(dep_last,dep)
        mask = dep_last.sign()
        mask_gt = dep_gt.sign()
        dep = self.scale_adjust(dep_last,dep)
    ...
    def forward(self,features, blur_depth, gt, rgb, stage):
        ...
        #outputs['scale'] = 1
        return outputs
class Iterative_Propagate2(Iterative_Propagate):
    def __init__(self,crop_h,crop_w,mode='c',dropout=False):
    ...
    def stage_forward(self,features,rgbd,dep_last,stage):
        ...
        dep = self.scale_adjust(dep_last,dep)
        dep_fusion = dep_last * mask + dep * (1-mask)
        feature_stage = torch.cat((feature_crop,dep_gt,dep_fusion),1)
        dep = model(feature_stage)
        dep = self.scale_adjust(dep_last,dep)
        return dep, feature_stage 
...
class Iterative_3DPropagate(Iterative_Propagate):
    ...
    def make_3D_feature(self,feature,dep):
        ...
        # mask_msmall_f = mask_msmall.float()
        # #>
